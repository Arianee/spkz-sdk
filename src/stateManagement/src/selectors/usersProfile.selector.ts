...
export const getUserProfileFromRoom = (parameters: { roomId: string, address: string }): UserProfile => {
  const {
    roomId,
    address
  } = parameters;
...
export const subcribeToOneProfileOfRoom = (parameters:{roomId, address:string}):Observable<UserProfile> => {
